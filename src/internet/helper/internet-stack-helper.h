...
class LatencyPerNode
{
public:
	int m_nodeNo;
	Ipv6Address m_addr;
	Ptr<Ipv6Interface> m_interface;
	Time m_latencyInitTime;	//for each node: time of starting first DAD query
	Time m_latencyInvalidTime;	//for each node: time of resolving query - the address is invalid (NOTE:
								//it may happen that for one node the address is both INVALID and PREFFERRED -
								//when new valid address was assigned later
	Time m_latencyPreferredTime;	//for each node: time of resolving query - the address is PREFERRED

	LatencyPerNode() : m_nodeNo(-1), m_addr("::"), m_interface(0), m_latencyInitTime(Seconds(-1.0)),
		  m_latencyInvalidTime(Seconds(-1.0)), m_latencyPreferredTime(Seconds(-1.0)) {}
};

class InternetStackHelper_Nd : public InternetStackHelper, public AsciiTraceHelperForIcmpv6
{
...
public:
  ...
  InternetStackHelper_Nd (int ndType = ND_BASIC/*, bool resultPrintInit = false*/) :
	  m_ndType(ndType),
	  m_first_mNStime(Seconds(-1.0)),
	  m_last_mNStime(Seconds(-1.0)),
	  m_first_mNDMessTime(Seconds(-1.0)),
	  m_last_mNDTtime(Seconds(-1.0))
	 // m_resultPrintInit(resultPrintInit)
  {};

  /**
   * Destroy the InternetStackHelper
   */
  virtual ~InternetStackHelper_Nd(void);
  //InternetStackHelper (const InternetStackHelper &);

  virtual void Install (Ptr<Node> node) const;

  void SetNdType(int type);


protected:

  //tracing methods and variables
  ...
  /**
	...
	*               when writing trace data.
	* @param prefix Filename prefix to use for ascii trace files.
	* @param icmpv6 Ptr to the Icmpv6 interface on which you want to enable tracing.
	*/
  ...
  typedef std::list<std::vector<int> *> DadStatsMessageType;
  typedef std::list<std::vector<int> *>::iterator DadStatsMessageTypeI;
  typedef std::list<std::vector<int> *> DadStatsMprOptType;
  typedef std::list<std::vector<int> *> DadStatsExitCode;
  typedef std::list<std::vector<int> *>::iterator DadStatsExitCodeI;
  typedef std::list<std::vector<int> *> DadStatsEntryCode;
  DadStatsMessageType m_txDadStatsMessageType;
  DadStatsMessageType m_rxDadStatsMessageType;
  DadStatsExitCode m_exitCodeDadStats;

  typedef std::list<LatencyPerNode *>::iterator LatenciesI;
  std::list<LatencyPerNode *> m_latencies;

  //per network stats
  Time m_first_mNStime;	//time of sending first mNS message
  Time m_last_mNStime;//time when the last mNS was received
  Time m_first_mNDMessTime;	//time of sending first multihop ND++ message (NS or NA, should be NS)
   Time m_last_mNDTtime;//time when the last multihop ND++ message was received (mNS or mNA)
...
public:
  ...
  void PrintResultsTxRxStats(std::string additionalInfo);
  void ResetDadStats();

  //enabling/disabling data collection
  void EnableIcmpv6AsciiTracing();
  void DisableIcmpv6AsciiTracing();
};
