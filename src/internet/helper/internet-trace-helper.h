...
/**
 * @brief Base class providing common user-level ascii trace operations for
 * helpers representing ICMPv6 protocols .
 */
class AsciiTraceHelperForIcmpv6
{
public:
  /**
   * @brief Construct an AsciiTraceHelperForIpv6.
   */
  AsciiTraceHelperForIcmpv6 () {}

  /**
   * @brief Destroy an AsciiTraceHelperForIcmpv6
   */
  virtual ~AsciiTraceHelperForIcmpv6 () {}

  /**
   * @brief Enable ascii trace output on the indicated Icmpv6 and interface pair.
   * @internal
   *
   * The implementation is expected to use a provided Ptr<OutputStreamWrapper>
   * if it is non-null.  If the OutputStreamWrapper is null, the implementation
   ...
   * each net device using the rules described in the class overview.
   *
   * If the prefix is provided, there will be one file per Icmpv6 and interface pair
   * created.  In this case, adding a trace context to the file would be pointless,
   * so the helper implementation is expected to TraceConnectWithoutContext.
   *
   * If the output stream object is provided, there may be many different Icmpv6
   * and interface pairs writing to a single file.  In this case, the trace
   * context could be important, so the helper implementation is expected to
   * TraceConnect.
   *
   ...
   *               when writing trace data.
   * @param prefix Filename prefix to use for ascii trace files.
   * @param icmpv6 Ptr<Icmpv6L4Protocol> on which you want to enable tracing.
   * @param interface The interface on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  virtual void EnableAsciiIcmpv6Internal (Ptr<OutputStreamWrapper> stream,
                                        std::string prefix,
                                        Ptr<Icmpv6L4Protocol> icmpv6,
                                        bool explicitFilename) = 0;

  virtual void EnableAsciiIcmpv6Internal (Ptr<Icmpv6L4Protocol> icmpv6,
                                          bool explicitFilename) = 0;
  /**
   * @brief Enable ascii trace output on the indicated Icmpv6 and interface pair.
   *
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
 // void EnableAsciiIcmpv6 (std::string prefix, Ptr<Icmpv6L4Protocol> icmpv6, uint32_t interface, bool explicitFilename = false);
   ...
   * @param interface The interface on which you want to enable tracing.
   */
 // void EnableAsciiIcmpv6 (Ptr<OutputStreamWrapper> stream, Ptr<Icmpv6L4Protocol> icmpv6, uint32_t interface);

  /**
   * @brief Enable ascii trace output the indicated Icmpv6 and interface pair
   * using an Icmpv6 previously named using the ns-3 object name service.
   *
   * @param prefix filename prefix to use for ascii files.
   * @param icmpv6Name The name of the Icmpv6 on which you want to enable tracing.
   * @param interface The interface on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
 // void EnableAsciiIcmpv6 (std::string prefix, std::string icmpv6Name, uint32_t interface, bool explicitFilename = false);
   ...
   * @param interface The interface on which you want to enable tracing.
   */
 // void EnableAsciiIcmpv6 (Ptr<OutputStreamWrapper> stream, std::string icmpv6Name, uint32_t interface);

  /**
   * @brief Enable ascii trace output on each Icmpv6 and interface pair in the
   * container
   *
   * @param prefix Filename prefix to use for ascii files.
   * @param c Icmpv6InterfaceContainer of Icmpv6 and interface pairs on which to
   *          enable tracing.
   */
 // void EnableAsciiIcmpv6 (std::string prefix, Icmpv6InterfaceContainer c);
   ...
   *          enable tracing.
   */
 // void EnableAsciiIcmpv6 (Ptr<OutputStreamWrapper> stream, Icmpv6InterfaceContainer c);

  /**
   * @brief Enable ascii trace output on all Icmpv6 and interface pairs existing
   * in the nodes provided in the container.
   *
   * \param prefix Filename prefix to use for ascii files.
   * \param n container of nodes.
   */
  void EnableAsciiIcmpv6 (std::string prefix, NodeContainer n);
   ...
   * \param n container of nodes.
   */
  void EnableAsciiIcmpv6 (Ptr<OutputStreamWrapper> stream, NodeContainer n);

  /**
   * @brief Enable pcap output on the Icmpv6 and interface pair specified by a
   * global node-id (of a previously created node) and interface.  Since there
   * can be only one Icmpv6 aggregated to a node, the node-id unambiguously
   * determines the Icmpv6.
   *
   * @param prefix Filename prefix to use when creating ascii trace files
   * @param nodeid The node identifier/number of the node on which to enable
   *               ascii tracing
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  void EnableAsciiIcmpv6 (std::string prefix, uint32_t nodeid, bool explicitFilename);
   ...
   * @param interface The interface on which you want to enable tracing.
   */
  void EnableAsciiIcmpv6 (Ptr<OutputStreamWrapper> stream, uint32_t nodeid);
   ...
   * @param prefix Filename prefix to use for ascii files.
   */
  void EnableAsciiIcmpv6All (std::string prefix);
  void EnableAsciiIcmpv6All ();


  void EnableAsciiIcmpv6All (std::string prefix, std::string sufix);
   ...
   *               when writing trace data.
   */
  void EnableAsciiIcmpv6All (Ptr<OutputStreamWrapper> stream);

private:
  /**
   * @internal Avoid code duplication.
   */
  void EnableAsciiIcmpv6Impl (Ptr<OutputStreamWrapper> stream,
                            std::string prefix,
                            uint32_t nodeid,
                            bool explicitFilename);
   ...
   * @internal Avoid code duplication.
   */
  void EnableAsciiIcmpv6Impl (Ptr<OutputStreamWrapper> stream, std::string prefix, NodeContainer n);
  void EnableAsciiIcmpv6Impl (NodeContainer n);

  /**
   * @internal Avoid code duplication.
   */
//  void EnableAsciiIcmpv6Impl (Ptr<OutputStreamWrapper> stream, std::string prefix, Icmpv6InterfaceContainer c);
