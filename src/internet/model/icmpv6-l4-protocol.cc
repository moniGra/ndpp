...
TypeId Icmpv6L4Protocol::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Icmpv6L4Protocol")
   ...
   .AddAttribute ("DadDelay", "Random delay before DAD start",
					  StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1.0]"),	//default was the Icmpv6L4Protocol::MAX_RTR_SOLICITATION_DELAY
					  MakePointerAccessor (&Icmpv6L4Protocol::m_initDadDelay),
  ...
  : m_node (0)
{
 ...
 /* m_dadDelay = CreateObject<UniformRandomVariable> ();
  m_dadDelay->SetAttribute ("Min", DoubleValue (0.0));
  m_dadDelay->SetAttribute ("Max", DoubleValue (Icmpv6L4Protocol::MAX_RTR_SOLICITATION_DELAY));*/
}
...
int64_t Icmpv6L4Protocol::AssignStreams (int64_t stream)
{
  ...
  m_initDadDelay->SetStream (stream);
  return 2;
}

double Icmpv6L4Protocol::GetDadDelay()
{
	return m_initDadDelay->GetValue();
}

/*double Icmpv6L4Protocol::GetInitDadDelay()
{
	return m_dadDelay->GetValue();
}*/
...
void Icmpv6L4Protocol::DoDAD (Ipv6Address target, Ptr<Ipv6Interface> interface)
{
  ...
  //this jitter doeas not work well here!!! - suppose we start DAD, we enter here and wait, but in teh external function there is
  //timer already set-up specifying when to finish DAD - with this delay DAD interval is in fact smaller than expected!!!
  //!!! thsi delay should be put in AddAddress, to schedule DAD!!!
...
enum IpL4Protocol::RxStatus Icmpv6L4Protocol::Receive (Ptr<Packet> packet, Ipv6Header const &header, Ptr<Ipv6Interface> interface)
{
  ...
  switch (type) 
    ...
    case Icmpv6Header::ICMPV6_ND_NEIGHBOR_SOLICITATION:
      HandleNS (p, header, header.GetSourceAddress(), header.GetDestinationAddress(), interface);
      break;
    case Icmpv6Header::ICMPV6_ND_NEIGHBOR_ADVERTISEMENT:
      HandleNA (p, header, header.GetSourceAddress(), header.GetDestinationAddress(), interface);
      break;
...
void Icmpv6L4Protocol::HandleNS (Ptr<Packet> packet, Ipv6Header ip, Ipv6Address const &src, Ipv6Address const &dst, Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  Icmpv6NS nsHeader ("::");
  ...
  if(tentative_dad)
  {
	  ...
	  ifaddr.SetState(Ipv6InterfaceAddress::INVALID);
	  NS_LOG_INFO(this << ", Somebody else is doing DAD on my tentative address: " << ifaddr.GetAddress() <<
  ...
  Ptr<Packet> p = ForgeNA (target.IsLinkLocal () ? interface->GetLinkLocalAddress ().GetAddress () : ifaddr.GetAddress (), src.IsAny () ? Ipv6Address::GetAllNodesMulticast () : src, target, &hardwareAddress, flags );
...
void Icmpv6L4Protocol::HandleNA (Ptr<Packet> packet, Ipv6Header ip, Ipv6Address const &src, Ipv6Address const &dst, Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  Icmpv6NA naHeader;
  Icmpv6OptionLinkLayerAddress lla (1);
          ...
          if (ifaddr.GetState () == Ipv6InterfaceAddress::TENTATIVE || ifaddr.GetState () == Ipv6InterfaceAddress::TENTATIVE_OPTIMISTIC)
            {
              ...
              NS_LOG_INFO(this << ", DAD detected duplicated address: " << ifaddr.GetAddress() << ". The address is INVALID");
            }
        }
...
Ptr<Packet> Icmpv6L4Protocol::ForgeNA (Ipv6Address src, Ipv6Address dst, Ipv6Address target, Address* hardwareAddress, uint8_t flags,
...
Ptr<Packet> Icmpv6L4Protocol::ForgeNS (Ipv6Address src, Ipv6Address dst, Ipv6Address target, Address hardwareAddress)
{
  ...
  /* if the source is unspec, multicast the NS to solicited-nodes multicast */
  if (src.IsAny() && !dst.IsSolicitedMulticast())
    {
...
void Icmpv6L4Protocol::FunctionDadTimeout (Ptr<Ipv6Interface> interface, Ipv6Address addr)
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  if (found && ifaddr.GetState () != Ipv6InterfaceAddress::INVALID)
    {
       ...
       */
      Ptr<Ipv6> ipv6 = this->m_node->GetObject<Ipv6> ();

      if (!ipv6->IsForwarding (ipv6->GetInterfaceForDevice (interface->GetDevice ())) && addr.IsLinkLocal ())
        {
          /* \todo Add random delays before sending RS - this is done - in SendRS there is a solicitation delay added
	  ...
	  NS_LOG_DEBUG("DAD time elapsed: no interface address found for " << addr << ". ");
}
...
//MG: original implementation here:
