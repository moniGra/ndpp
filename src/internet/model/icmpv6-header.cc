...
NS_OBJECT_ENSURE_REGISTERED (Icmpv6OptionMprParams);

TypeId Icmpv6OptionMprParams::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Icmpv6OptionMprParams")
    .SetParent<Icmpv6OptionHeader> ()
    .AddConstructor<Icmpv6OptionMprParams> ()
  ;
  return tid;
}

TypeId Icmpv6OptionMprParams::GetInstanceTypeId () const
{
  return GetTypeId ();
}

Icmpv6OptionMprParams::Icmpv6OptionMprParams (uint8_t willingness, uint8_t link_code)
	: m_willingness(willingness),
	  m_linkCode(link_code)
{
  SetType (Icmpv6Header::ICMPV6_OPT_MPR_PARAMS);
  m_reserved = (uint32_t)0;
  SetLength(GetSerializedSize()/8);
}

/*Icmpv6OptionMprParams::Icmpv6OptionMprParams(const Icmpv6OptionMprParams &o)
	: m_NeighAddresses(o.m_NeighAddresses),
	  m_len(o.m_len),
	  m_linkCode(o.m_linkCode),
...
Icmpv6OptionMprParams::~Icmpv6OptionMprParams ()
{
	m_NeighAddresses.clear();
}

void Icmpv6OptionMprParams::AddNeighbor(Ipv6Address addr)
{
	m_NeighAddresses.push_back(addr);
	SetLength(GetSerializedSize()/8);
}


void Icmpv6OptionMprParams::Print (std::ostream& os) const
{
  os << "( type = " << (uint32_t)GetType () << " length = " << (uint32_t)GetLength () << " link code = " << m_linkCode
		  << " willingness = " << m_willingness << " ";
  int i = 1;
  for (AddrListCI it = m_NeighAddresses.begin (); it != m_NeighAddresses.end (); ++it)
  {
	  os << "neighbor address " << i << ": " << *it << ", ";
  }
  os << ") ";
}

uint32_t Icmpv6OptionMprParams::GetSerializedSize () const
{
  uint32_t nb = 2 + 6 + m_NeighAddresses.size() * 16;	//in bytes
  return nb;
}

void Icmpv6OptionMprParams::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION_NOARGS ();
  uint8_t buff_addr[16];
  Buffer::Iterator i = start;

  i.WriteU8 (GetType ());
  i.WriteU8 (GetLength ());
  i.WriteU8(m_willingness);
  i.WriteU8(m_linkCode);
  i.WriteU32(m_reserved);

  for (AddrListCI it = m_NeighAddresses.begin (); it != m_NeighAddresses.end (); ++it)
  {
	it->Serialize(buff_addr);
	i.Write(buff_addr, 16);
  }

}

uint32_t Icmpv6OptionMprParams::Deserialize (Buffer::Iterator start)
{
	uint8_t buf[16];
	Buffer::Iterator i = start;
	Ipv6Address addr;
...
NS_OBJECT_ENSURE_REGISTERED (Icmpv6OptionMprAnnouncement);

TypeId Icmpv6OptionMprAnnouncement::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Icmpv6OptionMprAnnouncement")
    .SetParent<Icmpv6OptionHeader> ()
    .AddConstructor<Icmpv6OptionMprAnnouncement> ()
  ;
  return tid;
}

TypeId Icmpv6OptionMprAnnouncement::GetInstanceTypeId () const
{
  return GetTypeId ();
}

Icmpv6OptionMprAnnouncement::Icmpv6OptionMprAnnouncement ()
{
  SetType (Icmpv6Header::ICMPV6_OPT_MPR_ANNOUNCEMENT);
  m_reserved1 = (uint16_t)0;
  m_reserved2 = (uint32_t)0;
  SetLength(GetSerializedSize()/8);
}
...
Icmpv6OptionMprAnnouncement::~Icmpv6OptionMprAnnouncement ()
{
	m_MprAddresses.clear();
}

void Icmpv6OptionMprAnnouncement::AddMPR(Ipv6Address addr)
{
	m_MprAddresses.push_back(addr);
	SetLength(GetSerializedSize()/8);
}


void Icmpv6OptionMprAnnouncement::Print (std::ostream& os) const
{
  os << "( type = " << (uint32_t)GetType () << " length = " << (uint32_t)GetLength () << " ";
  int i = 1;
  for (AddrListCI it = m_MprAddresses.begin (); it != m_MprAddresses.end (); ++it)
  {
	  os << "mpr address " << i << ": " << *it << ", ";
  }
  os << ") ";
...
uint32_t Icmpv6OptionMprAnnouncement::GetSerializedSize () const
{
  uint32_t nb = 2 + 6 + m_MprAddresses.size() * 16;	//in bytes
  return nb;
}

void Icmpv6OptionMprAnnouncement::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION_NOARGS ();
  uint8_t buff_addr[16];
  Buffer::Iterator i = start;
  ...
  i.WriteU8 (GetType ());
  i.WriteU8 (GetLength ());
  i.WriteU16(m_reserved1);
  i.WriteU32(m_reserved2);
...
uint32_t Icmpv6OptionMprAnnouncement::Deserialize (Buffer::Iterator start)
{
	uint8_t buf[16];
	Buffer::Iterator i = start;
	Ipv6Address addr;
	...
	SetType (i.ReadU8 ());
	SetLength (i.ReadU8 ());
	m_reserved1 = i.ReadU16 ();
	m_reserved2 = i.ReadU32 ();
...
/////////////////////////////////////////
// MPR Announcement with Random IDs
//////////////////////////////////////////

NS_OBJECT_ENSURE_REGISTERED (Icmpv6OptionMprAnnWithRandomIds);

TypeId Icmpv6OptionMprAnnWithRandomIds::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Icmpv6OptionMprAnnWithRandomIds")
    .SetParent<Icmpv6OptionHeader> ()
    .AddConstructor<Icmpv6OptionMprAnnWithRandomIds> ()
  ;
  return tid;
}

TypeId Icmpv6OptionMprAnnWithRandomIds::GetInstanceTypeId () const
{
  return GetTypeId ();
}

Icmpv6OptionMprAnnWithRandomIds::Icmpv6OptionMprAnnWithRandomIds ()
{
  SetType (Icmpv6Header::ICMPV6_OPT_MPR_ANNOUNCEMENT);
  m_reserved1 = (uint16_t)0;
  m_reserved2 = (uint32_t)0;
...
void Icmpv6OptionMprAnnWithRandomIds::AddMPR(Ipv6AddrRandomIDPair addr)
{
	m_MprAddresses.push_back(addr);
	SetLength(GetSerializedSize()/8);
}
...
void Icmpv6OptionMprAnnWithRandomIds::Print (std::ostream& os) const
{
  os << "( type = " << (uint32_t)GetType () << " length = " << (uint32_t)GetLength () << " ";
  int i = 1;
  for (AddrListWithIdsCI it = m_MprAddresses.begin (); it != m_MprAddresses.end (); ++it, i++)
  {
	  os << "mpr address " << i << ": " << *it << "(randomID = " << it->m_randomID << "), ";
  }
  os << ") ";
}

uint32_t Icmpv6OptionMprAnnWithRandomIds::GetSerializedSize () const
{
	int addrSize = m_MprAddresses.size() * 20;
	uint32_t nb = 2 + 6 + addrSize;	//in bytes
	if(addrSize % 8)
	{
		//there was a padding
		nb += 4;	//add 4 octets (4 bytes)
   }

  return nb;
}

void Icmpv6OptionMprAnnWithRandomIds::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION_NOARGS ();
  uint8_t buff_addr[16];
  Buffer::Iterator i = start;
  ...
  for (AddrListWithIdsCI it = m_MprAddresses.begin (); it != m_MprAddresses.end (); ++it)
  {
	it->Serialize(buff_addr);
	i.Write(buff_addr, 16);
	i.WriteU32(it->m_randomID);
  }

  //add padding if needed
   if((m_MprAddresses.size() * 20) % 8)
   {
 	  i.WriteU32 (0); /* padding */;	//4 octets
   }

}

uint32_t Icmpv6OptionMprAnnWithRandomIds::Deserialize (Buffer::Iterator start)
{
	uint8_t buf[16];
	Buffer::Iterator i = start;
	Ipv6Address addr;
	...
	int nrOfAddr = ((GetLength() * 8) - 8) / 20;
	while(nrOfAddr > 0)
	{
		i.Read(buf, 16);
		addr.Set(buf);
		randomId = i.ReadU32();
		m_MprAddresses.push_back(Ipv6AddrRandomIDPair(addr,randomId));
...
NS_OBJECT_ENSURE_REGISTERED (Icmpv6OptionMprParamsWithRandomIds);

TypeId Icmpv6OptionMprParamsWithRandomIds::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Icmpv6OptionMprParamsWithRandomIds")
    .SetParent<Icmpv6OptionHeader> ()
    .AddConstructor<Icmpv6OptionMprParamsWithRandomIds> ()
  ;
  return tid;
}

TypeId Icmpv6OptionMprParamsWithRandomIds::GetInstanceTypeId () const
{
  return GetTypeId ();
}

Icmpv6OptionMprParamsWithRandomIds::Icmpv6OptionMprParamsWithRandomIds (uint8_t willingness, uint8_t link_code)
	: m_willingness(willingness),
	  m_linkCode(link_code)
{
  SetType (Icmpv6Header::ICMPV6_OPT_MPR_PARAMS);
...
Icmpv6OptionMprParamsWithRandomIds::~Icmpv6OptionMprParamsWithRandomIds ()
{
	m_NeighAddresses.clear();
}

void Icmpv6OptionMprParamsWithRandomIds::AddNeighbor(Ipv6AddrRandomIDPair addr)
{
	m_NeighAddresses.push_back(addr);
	SetLength(GetSerializedSize()/8);
}
...
void Icmpv6OptionMprParamsWithRandomIds::Print (std::ostream& os) const
{
  os << "( type = " << (uint32_t)GetType () << " length = " << (uint32_t)GetLength () << " link code = " << m_linkCode
		  << " willingness = " << m_willingness << " ";
  int i = 1;
  for (AddrListWithIdsCI it = m_NeighAddresses.begin (); it != m_NeighAddresses.end (); ++it, i++)
  {
	  os << "neighbor address " << i << ": " << *it << "(randomID = " << it->m_randomID << "), ";
  }
  os << ") ";
}

uint32_t Icmpv6OptionMprParamsWithRandomIds::GetSerializedSize () const
{
	int addrSize = m_NeighAddresses.size() * 20;
	uint32_t nb = 2 + 6 + addrSize;	//in bytes
	if(addrSize % 8)
	{
		//there was a padding
...
void Icmpv6OptionMprParamsWithRandomIds::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION_NOARGS ();
  uint8_t buff_addr[16];
  Buffer::Iterator i = start;
  ...
  for (AddrListWithIdsCI it = m_NeighAddresses.begin (); it != m_NeighAddresses.end (); ++it)
  {
	it->Serialize(buff_addr);
	i.Write(buff_addr, 16);
	i.WriteU32(it->m_randomID);
...
uint32_t Icmpv6OptionMprParamsWithRandomIds::Deserialize (Buffer::Iterator start)
{
	uint8_t buf[16];
	Buffer::Iterator i = start;
	Ipv6Address addr;
	...
	while(nrOfAddr > 0)
	{
		...
		addr.Set(buf);
		randomId = i.ReadU32();
		m_NeighAddresses.push_back(Ipv6AddrRandomIDPair(addr,randomId));
