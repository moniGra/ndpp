...
//MG: modified! - only VALID address can be selected
  ...
  for (Ipv6InterfaceAddressListCI it = m_addresses.begin (); it != m_addresses.end (); ++it)
    {
      ...
      if ((*it).GetAddress ().IsLinkLocal () && (*it).GetState() != Ipv6InterfaceAddress::INVALID
    		  && (*it).GetState() != Ipv6InterfaceAddress::TENTATIVE)
        {
          return (*it);
        }
    }
  NS_LOG_DEBUG("No valid link-local address on interface " << this);
