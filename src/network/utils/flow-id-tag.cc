...
NS_OBJECT_ENSURE_REGISTERED(SeqNoTag);
...
TypeId
SeqNoTag::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SeqNoTag")
    .SetParent<FlowIdTag> ()
    .AddConstructor<SeqNoTag> ()
  ;
  return tid;
}

TypeId
SeqNoTag::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

SeqNoTag::SeqNoTag (uint32_t id)
  : FlowIdTag (id)
{
}
