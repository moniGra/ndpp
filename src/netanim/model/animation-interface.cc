...
double AnimationInterface_Ndpp::maxRange;
...
AnimationInterface::~AnimationInterface ()
{
  if (userBoundary)
    {
  //    delete userBoundary;	//MG: this was crashing during runtime!!!
    }
  StopAnimation ();
}
...
//AnimationInterface_Ndpp:

AnimationInterface_Ndpp::AnimationInterface_Ndpp(const std::string fn, uint64_t maxPktsPerFile, bool usingXML)
{
	m_routingF = 0;
	m_xml = usingXML;
	...
	m_maxPktsPerFile = maxPktsPerFile;
...
void AnimationInterface_Ndpp::StartAnimation (bool restart)
{
  m_currentPktCount = 0;
  m_started = true;
  SetOutputFile (m_outputFileName);
  ...
  Ptr <MobilityModel> mobility_src;
  Ptr<Node> n;
  NodeList::Iterator k;
  uint32_t n1Id, n2Id;

  NS_LOG_INFO ("Custom: Setting topology links");
  // Now dump the p2p links
	for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End (); i++)
	{
	  n = *i;
	  k = i;
	  n1Id = n->GetId ();
	  mobility_src = n->GetObject <MobilityModel> ();
	 // uint32_t nDev = n->GetNDevices ();  // Number of devices
	  for (NodeList::Iterator l = ++k; l != NodeList::End (); l++)
	  {
		  if (mobility_src->GetDistanceFrom ((*l)->GetObject <MobilityModel> ()) <= maxRange)
		  {
			  //link exists - record it
			  n2Id = (*l)->GetId ();
			  NS_LOG_INFO ("Link: node " << n1Id << " to node " << n2Id);
			  SetLinkDescription (n1Id, n2Id, "","", "");
			  //AddToIpv4AddressNodeIdTable (GetIpv4Address (dev), n1Id);
			 // AddToIpv4AddressNodeIdTable (GetIpv4Address (chDev), n2Id);
			  std::ostringstream oss;
			  if (m_xml)
  ...
  if (m_xml && !restart)
    {
      WriteN (GetXMLClose ("topology"), m_f);
      Simulator::Schedule (m_mobilityPollInterval, &AnimationInterface_Ndpp::MobilityAutoCheck, this);
    }
  if (!restart)
	  ConnectCallbacks ();
}

void AnimationInterface_Ndpp::SetMaxRangeForDrawingTopology(double range)
{
	maxRange = range;
}

void AnimationInterface_Ndpp::Cleanup()
{
	 if (userBoundary)
	{
      delete userBoundary;
...
/*void AnimationInterface_Ndpp::SetTopologyLinks(double maxRange)
{
	Ptr <MobilityModel> mobility_src;
	 Ptr<Node> n;
	NodeList::Iterator k;
