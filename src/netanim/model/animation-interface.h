...
public:
  ...
  //MG: change to allow for calling StartApplication from derived class
  AnimationInterface(){};
...
protected:
   ...
   */
  virtual void StartAnimation (bool restart = false);
...
class AnimationInterface_Ndpp : public AnimationInterface
{
private:
	static double maxRange;
public:
	AnimationInterface_Ndpp(const std::string filename,
			uint64_t maxPktsPerFile = MAX_PKTS_PER_TRACE_FILE,
			bool usingXML = true);
	~AnimationInterface_Ndpp(){}
	static void SetMaxRangeForDrawingTopology(double maxRange);
	static void Cleanup();
protected:
	void StartAnimation (bool restart = false);

	//void SetTopologyLinks(double maxRange);
};
