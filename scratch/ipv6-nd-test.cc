...
//  int targetNr = 5;
 ...
 // cmd.AddValue ("targetNr", "Select\"target\" of DAD - node nr from which we will assign the address to initiating node", targetNr);
  ...
  cmd.AddValue ("runNo", "Set run no. for random number generator", randomRunNo);
 ...
 // addresses->NewNetwork(Ipv6Address("fe80::"), Ipv6Prefix(64)); //w ns-3.18 adresy link-local przydzielane sa automatycznie
  ...
  //TODO - check if below is still true in 3.18 - DONE:
  	  //it si fixed - static values taht were making teh effect in 3.11 are gone now
  	  //calling SetStream again will reset the stream with the new run/seed value.
  	  //SetStream is called through a variety of AssignStreams functions (in helper classes)
  //initial set-up:
  //this allows to only set the run/seed before first RngStream object is created. Once it is done,
  //changes are not effective -> for one program instance once RngStreams are started you
  //cannot reset them!!!!!!!!!!!

  //NS-3 recommends to set runs, not seeds - this guarantees indepenency
  RngSeedManager::SetRun(randomRunNo);
  ...
  /* //trace topology changes - to checkif topology is the same for each run:
   ...
   if(ndType==1 || ndType==2)
   {
	...
	Simulator::Schedule(Seconds(15.2), AddAddress, wifiInterfaces, wifiDevices, 0);
	...
	/*}
		  ...
		  results.ResetAverageStatsSingleNodeRun();
		  //internetv6->Reset();
	  ...
	  results.PrintPartialResultHopLimit(resultType, inputCommand, hopLimit);
	  results.ResetAverageStats();
  }*/
