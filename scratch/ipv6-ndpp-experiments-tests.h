...
public:
	 ...
	 */
	 ...
	 void SetRandomNodes (std::unordered_map<int, Ipv6Address> & selectedNodes, std::unordered_map<int, DuplInfo> & selectedDuplNodes);	//keep track of selected nodes)
	 ...
	 public:
		 ...
		 bool m_duplAddress;
		 Average<double> m_averageLatencyINV;	//in case of address query resolved as INVALID
		 	 	 	 	 	 	 	 	 	 //NOTE: same node can have value for latency INV or
		 	 	 	 	 	 	 	 	 	 //latency PREF - but for DIFFERENT ADDRESSES (new auto address is on)
		 Average<double> m_averageLatencyPREF;	//in case of address query resolved as PREFERRED
		 Average<double> m_averageLatency;	//in case of address query resolved as either PREFERRED (if finished this way)
		 	 	 	 	 	 	 	 	 	 //OR INVALID if preferred state was never reached
		 Average<double> m_averageLatencyDupl;	//in case of address query for duplicated address
		 Average<double> m_averageLatencyNoDupl;	//in case of address query for not duplicated address
	 };

private:
	bool m_resultPrintInit;
	Ptr<OutputStreamWrapper> m_outputStream;
	Average<double> m_averageResult_mNSCount;
	Average<double> m_averageResult_mNDMessCount;
	Average<double> m_averageResult_mNSLife;
	Average<double> m_averageResult_mNDMessLife;
	...
	std::vector<int> m_selectedRandomNodes;	//keep track of selected nodes
	...
	std::map<int, LatencyInfo > m_averageLatencyPerNode;	//in milliseconds!!!; int to nr kolejnych wezlow w ramach jednego losowania
	std::list<LatencyInfo > m_averageLatencyPerRNSelection;	//in milliseconds!!!; int to nr kolejnych zestawow losowanych wezlow
...
private:
   ...
   Ipv6Address SetDuplTargetAddr(NetDeviceContainer* NetDevices, std::unordered_map<int, DuplInfo>::iterator it2, uint32_t initiator, uint32_t target);
  ...
  static const int m_maxNoOfRuns = 70;
 ...
 //simulation environment elements, helpers:
  ...
  std::unordered_map<int, Ipv6Address> m_selectedNodes;	//keep track of selected nodes
  ...
  std::unordered_map<int, DuplInfo> m_selectedNodesDupl;	//keep track of selected duplicated nodes
  std::unordered_map<int, DuplInfo> m_selectedNodesSameDupl;	//keep track of selected duplicated nodes
  bool m_newNodes;
...
void NdppExperiment::SelectRandomNodes(NetDeviceContainer* NetDevices)
{
	...
	std::unordered_map<int, Ipv6Address>::iterator it;
	std::unordered_map<int, DuplInfo>::iterator it2;
	std::unordered_map<int, DuplInfo>::iterator it3;
	...
	for(int i=0; i<m_noOfRandomNodes; i++)
	{
		...
		m_selectedNodes.insert(std::make_pair(currentInitiator, "::"));
	}
	...
	if(m_noOfDuplications)
	{
		...
		{
			for(it = m_selectedNodes.begin(); it!= m_selectedNodes.end(); it++)
				m_selectedNodesDupl.insert(std::make_pair(it->first, DuplInfo("::",-1)));
		}
		else
		{
			for(int i=0; i<m_noOfDuplications; i++)
			{
				do
				{
					...
					for(it = m_selectedNodes.begin(), j=0; it!= m_selectedNodes.end(); it++, j++)
					{
						if(j == currentNoInSelectedSet)
						{
					...
					//check if not selected before
					 ...
					 it2 = m_selectedNodesDupl.find(currentInitiator);
					 if(it2 == m_selectedNodesDupl.end())
						 break;
				}
				while (true);
				m_selectedNodesDupl.insert(std::make_pair(currentInitiator, DuplInfo("::",-1)));
			}
		}
		...
		if(m_noOfSameDupl)
		{
			...
			if(m_noOfDuplications == m_noOfSameDupl)	//select all nodes
			{
				for(it2 = m_selectedNodesDupl.begin(); it2!= m_selectedNodesDupl.end(); it2++)
				{
					...
					m_selectedNodesSameDupl.insert(std::make_pair(it2->first, DuplInfo(addr_target, target)));
				}
			}
			else
			{
				for(int i=0; i<m_noOfSameDupl; i++)
				{
					do
					{
						...
						for(it2 = m_selectedNodesDupl.begin(), j=0; it2!= m_selectedNodesDupl.end(); it2++, j++)
						{
							if(j == currentNoInSelectedSet)
							{
						...
						//check if not selected before
						 ...
						 it3 = m_selectedNodesSameDupl.find(currentInitiator);
						 if(it3 == m_selectedNodesSameDupl.end())
							 break;
					}
					while (true);

					addr_target = SetDuplTargetAddr(NetDevices,it2,currentInitiator,target);
					m_selectedNodesSameDupl.insert(std::make_pair(currentInitiator, DuplInfo(addr_target, target)));
				}
			}
...
Ipv6Address NdppExperiment::SetDuplTargetAddr(NetDeviceContainer* NetDevices, std::unordered_map<int, DuplInfo>::iterator it2, uint32_t initiator, uint32_t target)
{
	//set address:
	...
	std::unordered_map<int, Ipv6Address>::iterator it = m_selectedNodes.find(initiator);	//currentInitiator is the same as it2->first
	it->second = addr_target;
...
void NdppExperiment::AddAddressRandomNodes(NetDeviceContainer* NetDevices)
{
	...
	std::unordered_map<int, Ipv6Address>::const_iterator it;
...
void NdppExperiment::RemoveAddressRandomNodes(NetDeviceContainer* NetDevices)
{
	...
	for(it = m_selectedNodes.begin(); it!= m_selectedNodes.end(); it++)
	{
		...
		ipv6_init->RemoveAddress(ifIndex_init, it->second);
...
void NdppExperiment::SetInterfacesUp (NetDeviceContainer* NetDevices)
{
	...
	for (u_int32_t i = 0; i < NetDevices->GetN(); i++)
	{
		...
		if(m_ndType == InternetStackHelper_Nd::ND_NDPP ||  m_ndType == InternetStackHelper_Nd::ND_NDPP_FRW)
...
void NdppExperiment::SetInterfacesUpNewNodes(NetDeviceContainer* NetDevices, bool newNodes)
{
	...
	else	//set up "baseline" -> "old" nodes
	{
		for (u_int32_t i = 0; i < NetDevices->GetN(); i++)
		{
			if(m_newNodes && m_selectedNodes.find(i) != m_selectedNodes.end())
...
{
	m_anim = new AnimationInterface_Ndpp("/home/monika/Workspaces/ns-3-update-nd++/ns-3.18/simul_results/netAnim/netanim-" +
...
*/
	...
	*/
	...
	for (uint32_t i = 0; i < wifiNodes->GetN (); i++)
		m_anim->UpdateNodeColor( wifiNodes->Get(i), 0, 0, 0);	//black

	//oznacz wezly wybrane
	for(it = m_selectedNodes.begin(); it != m_selectedNodes.end(); it++)
		if(m_selectedNodesDupl.find(it->first) == m_selectedNodesDupl.end())
		{
	...
	for(it2 = m_selectedNodesDupl.begin(); it2 != m_selectedNodesDupl.end(); it2++)
		if(m_selectedNodesSameDupl.find(it2->first) == m_selectedNodesSameDupl.end())
		{
...
*/
	...
	/* for(int i=0; i<nWifi; i++)
	{
		...
		case RandomWaypoint:
			...
			//TODO: this has to be updated to specify position allocator for this mobility model
			 /*ObjectFactory pos;
			  ...
			  mobilityAdhoc.SetPositionAllocator (taPositionAlloc);	//to jest chyab niepotrzebne - to chyba ustawia polozenei poczatkowe, a ustawienei powyzej polozenei docelowe po ruchu wybierane w trakcie mobility
			  */
			break;
		case Grid:
		default:
	  ...
	  if(m_ndType == InternetStackHelper_Nd::ND_NDPP_FRW || m_ndType == InternetStackHelper_Nd::ND_NDPP)
	  {
	  ...
	  m_results.EnableResultsCollection("/home/monika/Workspaces/ns-3-update-nd++/ns-3.18/simul_results/ipv6-nd"
	 		  , m_resultFileSuffix);
	 ...
	 // AnimationInterface::SetBoundary(0, 0, 400, 400);
	...
	do
	{
		...
		if(needMoreRunsLat)
		{
			if(noOfRuns >= 30)
			{
				...
				NS_ASSERT_MSG(noOfRuns == 30, "Ndpp_Experiment: Sth. is wrong - no of runs is greater than max value for latency exp.");
...
void ResultsCollection::SetRandomNodes (std::unordered_map<int, Ipv6Address> & selectedNodes,
		std::unordered_map<int, DuplInfo> & selectedDuplNodes)
{
	m_selectedRandomNodes.clear();
	m_selectedDuplNodes.clear();
	//std::map<int, Ipv6Address>::const_iterator it;
	std::unordered_map<int, Ipv6Address>::const_iterator it;
	std::unordered_map<int, DuplInfo>::const_iterator it2;
...
void ResultsCollection::PrintIntroMultiExp(std::string additionalInfo)
{
	if(!m_resultPrintInit)
	{
		...
		*m_outputStream->GetStream () << "Results for multi experiment with random nodes" << std::endl
		...
		if(!m_selectedDuplNodes.empty())
			*m_outputStream->GetStream () << "Duplication probability \t\t\t\t\t\t\t | \tmNS Message Count" <<
					" \t\t\t\t\t\t\t\t\t\t | \tmND Total Message Count" << " \t\t\t\t\t\t\t\t\t\t | \tmNS Message Lifetime"
	...
	//	*m_outputStream->GetStream () << "Message Count:" << std::endl << std::endl << std::endl;
		...
		*m_outputStream->GetStream()<< "\t" << "|"  << "\t";
		...
		//Latency:
		*m_outputStream->GetStream() << "Avg.Latency (millisec.)" << "\t" << "Min" << "\t"
				<< "Max" << "\t" << "StdDev" << "\t" << "Avg.Latency-INV (millisec.)" << "\t" << "Min" << "\t"
				<< "Max" << "\t" << "StdDev" << "\t" << "Avg.Latency-PREF(millisec.)" << "\t" << "Min" << "\t"
				<< "Max" << "\t" << "StdDev" << "\t";

		if(m_selectedDuplNodes.size() != m_selectedRandomNodes.size())	//all nodes duplicated
		{
			*m_outputStream->GetStream() << "Avg.Latency Not Dupl. Nodes (millisec.)" << "\t" << "Min" << "\t"
					<< "Max" << "\t" << "StdDev" << "\t";
		}
		if(!m_selectedDuplNodes.empty())
		{
			*m_outputStream->GetStream() << "Avg.Latency Dupl. Nodes (millisec.)" << "\t" << "Min" << "\t"
					<< "Max" << "\t" << "StdDev" << "\t" ;
		}
...
void ResultsCollection::PrintResultHopLimit(std::string additionalInfo, uint32_t hopLimit, int noOfRuns, bool printTotal)
{
	if(!m_resultPrintInit)
	{
		...
		*m_outputStream->GetStream() << "HopLimit" << "\t" << "Average" << "\t" << "Min" << "\t"
				<< "Max" << "\t" << "StdDev" << "\t" << "Var" << "\t" << "noOfRuns" << std::endl;
	}

	*m_outputStream->GetStream() << hopLimit << "\t" << m_averageResult_mNSCount.Avg() << "\t" << m_averageResult_mNSCount.Min()
				<< "\t" << m_averageResult_mNSCount.Max() << "\t" << m_averageResult_mNSCount.Stddev() << "\t" << m_averageResult_mNSCount.Var()
				 << "\t" << noOfRuns << std::endl;
	...
	if(printTotal)
	{
		...
		*m_outputStream->GetStream() << hopLimit << "\t" << m_averageResult_mNDMessCount.Avg() << "\t" << m_averageResult_mNDMessCount.Min()
					<< "\t" << m_averageResult_mNDMessCount.Max() << "\t" << m_averageResult_mNDMessCount.Stddev() << "\t" << m_averageResult_mNDMessCount.Var()
					 << "\t" << noOfRuns << std::endl;
	}
		...
		*m_outputStream->GetStream()<< "\t";
		...
		for(it = m_selectedDuplNodes.begin(); it!= m_selectedDuplNodes.end(); it++)
					*m_outputStream->GetStream() << it->first << "->" << it->second.m_target
					<< "(" << ((double)it->second.m_dupCount) / count << ")" << ", ";
...
void ResultsCollection::PrintPartialResultLatencyRN(std::string additionalInfo, std::string lineEnd)
{
	...
	*m_outputStream->GetStream() << m_averageLatencyPerRNSelection.back().m_averageLatency.Avg() << "\t"
			...
			<<  m_averageLatencyPerRNSelection.back().m_averageLatency.Stddev() << "\t";
	//latency INV:
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Count() != 0)
		*m_outputStream->GetStream() << m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Avg() << "\t"
			...
			<<  m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Stddev() << "\t";
	else
		*m_outputStream->GetStream() << "-1" << "\t" << "-1" << "\t" <<  "-1" << "\t" << "-1" << "\t";

	//latency PREF
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Count() != 0)
		*m_outputStream->GetStream() << m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Avg() << "\t"
			...
			<<  m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Stddev() << "\t";
	else
		*m_outputStream->GetStream() << "-1" << "\t" << "-1" << "\t" <<  "-1" << "\t" << "-1" << "\t";
	...
	if(m_selectedDuplNodes.size() != m_selectedRandomNodes.size())	//all nodes duplicated
	{
		*m_outputStream->GetStream() << m_averageLatencyPerRNSelection.back().m_averageLatencyNoDupl.Avg() << "\t"
			...
			<<  m_averageLatencyPerRNSelection.back().m_averageLatencyNoDupl.Stddev() << "\t";
	}
	//laetncy Dupl
	if(!m_selectedDuplNodes.empty())
	{
		*m_outputStream->GetStream() << m_averageLatencyPerRNSelection.back().m_averageLatencyDupl.Avg() << "\t"
			...
			<<  m_averageLatencyPerRNSelection.back().m_averageLatencyDupl.Stddev() << "\t";
	}
	//details per node:
...
void ResultsCollection::PrintResultLatencyRN(std::string additionalInfo)
{
	...
	*m_outputStream->GetStream () << std::endl << std::endl << "Results for Latency experiment:" << std::endl;
	...
	//total header :
	*m_outputStream->GetStream() << "Average latency total (millisec.)" << "\t" << "Min" << "\t"
			<< "Max" << "\t" << "StdDev" << "\t" << "Var" << std::endl;
	//value:
	*m_outputStream->GetStream() << m_averageLatencyTotal.m_averageLatency.Avg() << "\t"
	...
	//INV header :
	*m_outputStream->GetStream() << "Average latency INV (millisec.)" << "\t" << "Min" << "\t"
			<< "Max" << "\t" << "StdDev" << "\t" << "Var" << std::endl;
	//value:
	if(m_averageLatencyTotal.m_averageLatencyINV.Count() != 0)
		...
		*m_outputStream->GetStream() << "-1" << "\t" << "-1" << "\t" <<  "-1" << "\t" << "-1" << std::endl;

	//PREF header :
	*m_outputStream->GetStream() << "Average latency PREF (millisec.)" << "\t" << "Min" << "\t"
			<< "Max" << "\t" << "StdDev" << "\t" << "Var" << std::endl;
	//value:
	if(m_averageLatencyTotal.m_averageLatencyPREF.Count() != 0)
	...
	if(m_selectedDuplNodes.size() != m_selectedRandomNodes.size())	//all nodes duplicated
	{
		*m_outputStream->GetStream() << "Average latency No Dupl. (millisec.)" << "\t" << "Min" << "\t"
			<< "Max" << "\t" << "StdDev" << "\t" << "Var" << std::endl;
	//value:
	...
	if(!m_selectedDuplNodes.empty())
	{
		*m_outputStream->GetStream() << "Average latency Dupl (millisec.)" << "\t" << "Min" << "\t"
				<< "Max" << "\t" << "StdDev" << "\t" << "Var" << std::endl;
		//value:
...
void ResultsCollection::PrintResultMessageLifetime(std::string additionalInfo, uint32_t hopLimit, int noOfRuns, bool printTotal)
{
	...
	*m_outputStream->GetStream() << hopLimit << "\t" << m_averageResult_mNSLife.Avg() << "\t" << m_averageResult_mNSLife.Min()
			<< "\t" << m_averageResult_mNSLife.Max() << "\t" << m_averageResult_mNSLife.Stddev() << "\t"
			<< m_averageResult_mNSLife.Var() << "\t" << noOfRuns << std::endl;
	...
	if(printTotal)
	{
		...
		*m_outputStream->GetStream() << hopLimit << "\t" << m_averageResult_mNDMessLife.Avg() << "\t" << m_averageResult_mNDMessLife.Min()
				<< "\t" << m_averageResult_mNDMessLife.Max() << "\t" << m_averageResult_mNDMessLife.Stddev() << "\t"
				<< m_averageResult_mNDMessLife.Var() << "\t" << noOfRuns << std::endl;
	}
...
{
	m_averageResult_mNSCount.Update(m_average_mNSolCount_singleRun.Avg());
	m_averageResult_mNDMessCount.Update(m_average_mNDMessCount_singleRun.Avg());
			...
			it != internetHelper->m_latencies.end();it++)
	{
		if( (*it)->m_latencyInitTime >= Seconds(0.0))
		{
			if((*it)->m_latencyInvalidTime >= Seconds(0.0))
			{
				...
				currLatency = ((*it)->m_latencyInvalidTime - (*it)->m_latencyInitTime).GetMilliSeconds();
				insertResult.first->second.m_averageLatencyINV.Update(currLatency);
			}
			if ((*it)->m_latencyPreferredTime >= Seconds(0.0))
			{
				...
				insertResult = m_averageLatencyPerNode.insert(std::make_pair((*it)->m_nodeNo, LatencyInfo()));
				currLatency = ((*it)->m_latencyPreferredTime - (*it)->m_latencyInitTime).GetMilliSeconds();
				(insertResult.first)->second.m_averageLatencyPREF.Update(currLatency);
			}
			else
			...
			(insertResult.first)->second.m_averageLatency.Update(currLatency);
...
void ResultsCollection::UpdateLatencyStatsRN()
{
	...
	{
		if(it->second.m_averageLatencyINV.Count())
			m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Update(it->second.m_averageLatencyINV.Avg());
		if(it->second.m_averageLatencyPREF.Count())
			m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Update(it->second.m_averageLatencyPREF.Avg());
		m_averageLatencyPerRNSelection.back().m_averageLatency.Update(it->second.m_averageLatency.Avg());

		if(it->second.m_duplAddress)
			m_averageLatencyPerRNSelection.back().m_averageLatencyDupl.Update(it->second.m_averageLatency.Avg());
		else
			m_averageLatencyPerRNSelection.back().m_averageLatencyNoDupl.Update(it->second.m_averageLatency.Avg());
	}

	//update final stats - building step by step an average over all positions in m_averageLatencyPerRNSelection:
	m_averageLatencyTotal.m_averageLatency.Update(m_averageLatencyPerRNSelection.back().m_averageLatency.Avg());
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Count())
		m_averageLatencyTotal.m_averageLatencyINV.Update(m_averageLatencyPerRNSelection.back().m_averageLatencyINV.Avg());
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Count())
		m_averageLatencyTotal.m_averageLatencyPREF.Update(m_averageLatencyPerRNSelection.back().m_averageLatencyPREF.Avg());
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyDupl.Count())
		m_averageLatencyTotal.m_averageLatencyDupl.Update(m_averageLatencyPerRNSelection.back().m_averageLatencyDupl.Avg());
	if(m_averageLatencyPerRNSelection.back().m_averageLatencyNoDupl.Count())
		m_averageLatencyTotal.m_averageLatencyNoDupl.Update(m_averageLatencyPerRNSelection.back().m_averageLatencyNoDupl.Avg());
...
{
	m_averageResult_mNSLife.Update(m_average_mNSlife_singleRun.Avg());
	m_averageResult_mNDMessLife.Update(m_average_mNDMessLife_singleRun.Avg());
